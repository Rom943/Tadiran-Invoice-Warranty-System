...
exports.validateImageFile = exports.verifyInstallationDate = exports.validateWarrantyByOCR = exports.datesMatch = exports.validateInvoiceDateRange = exports.extractDatesFromText = exports.extractTextWithFallback = exports.extractTextFromImage = void 0;
...
try {
    ...
    if (env_config_1.default.googleVision.keyFilePath) {
        try {
            // Try to decode base64 credentials
            const decodedCredentials = Buffer.from(env_config_1.default.googleVision.keyFilePath, 'base64').toString('utf-8');
            const credentials = JSON.parse(decodedCredentials);
            visionClient = new vision_1.ImageAnnotatorClient({
                credentials: credentials,
                projectId: env_config_1.default.googleVision.projectId || credentials.project_id,
            });
            ...
            if (fs_1.default.existsSync(env_config_1.default.googleVision.keyFilePath)) {
                visionClient = new vision_1.ImageAnnotatorClient({
                    keyFilename: env_config_1.default.googleVision.keyFilePath,
                    projectId: env_config_1.default.googleVision.projectId,
                });
                console.log('Google Cloud Vision initialized with key file');
            }
            else {
                throw new Error(`Invalid base64 credentials or file not found: ${env_config_1.default.googleVision.keyFilePath}`);
            }
        }
    }
    else {
...
const extractTextFromImage = async (imagePath) => {
    ...
    const validation = await (0, exports.validateImageFile)(imagePath);
    if (!validation.isValid) {
        throw new Error(`Image validation failed: ${validation.error}`);
    }
    console.log(`Processing image with Google Vision: ${path_1.default.basename(imagePath)} (${Math.round(validation.fileSize / 1024)}KB)`);
    try {
        // Read the image file
        ...
        const [result] = await visionClient.textDetection({
            ...
            imageContext: {
                languageHints: env_config_1.default.googleVision.languageHints,
            },
        });
 ...
 */
const extractTextWithFallback = async (imagePath) => {
...
exports.extractTextWithFallback = extractTextWithFallback;
/**
...
const extractDatesFromText = (text) => {
    ...
    for (const pattern of dateRegexPatterns) {
        ...
        for (const match of matches) {
            ...
            for (const format of dateFormats) {
                try {
                    const date = (0, date_fns_1.parse)(cleanMatch, format, new Date());
    ...
    for (const sequence of numberSequences) {
        ...
        for (const interpretation of interpretations) {
            ...
            for (const format of dateFormats) {
                try {
                    const date = (0, date_fns_1.parse)(interpretation, format, new Date());
                    if ((0, date_fns_1.isValid)(date)) {
                        const year = date.getFullYear();
                        if (year >= 1900 && year <= 2030) {
        ...
        for (const match of matches) {
            ...
            for (const dateStr of dateStrings) {
                for (const format of ['dd/MM/yyyy', 'MM/dd/yyyy']) {
                    try {
                        const date = (0, date_fns_1.parse)(dateStr, format, new Date());
                        if ((0, date_fns_1.isValid)(date)) {
    ...
    const uniqueDates = Array.from(new Set(extractedDates.map(d => d.toISOString()))).map(dateStr => new Date(dateStr));
 ...
 */
const validateInvoiceDateRange = (installationDate, extractedDates) => {
    ...
    for (const extractedDate of extractedDates) {
        const daysDiff = Math.abs((0, date_fns_1.differenceInDays)(extractedDate, installationDate));
...
exports.validateInvoiceDateRange = validateInvoiceDateRange;
/**
 * Compare dates ignoring time component
 */
const datesMatch = (date1, date2) => {
    return (0, date_fns_1.isEqual)(new Date(date1.getFullYear(), date1.getMonth(), date1.getDate()), new Date(date2.getFullYear(), date2.getMonth(), date2.getDate()));
};
 ...
 */
const validateWarrantyByOCR = async (imagePath, installationDate) => {
    try {
        console.log(`Starting Vision API validation for installation date: ${installationDate.toISOString()}`);
        // Extract text from image using fallback method
        const ocrText = await (0, exports.extractTextWithFallback)(imagePath);
        ...
        const extractedDates = (0, exports.extractDatesFromText)(ocrText);
        console.log(`Extracted ${extractedDates.length} dates from Vision API text:`, extractedDates.map(d => d.toISOString()));
        // Validate date range
        const validation = (0, exports.validateInvoiceDateRange)(installationDate, extractedDates);
...
exports.validateWarrantyByOCR = validateWarrantyByOCR;
/**
 ...
 */
const verifyInstallationDate = async (imageUrl, claimedDate) => {
    try {
        // Extract text from image
        const ocrText = await (0, exports.extractTextFromImage)(imageUrl);
        // Extract dates from text
        const extractedDates = (0, exports.extractDatesFromText)(ocrText);
        // No dates found
        if (extractedDates.length === 0) {
        ...
        const hasMatch = extractedDates.some(date => (0, exports.datesMatch)(date, claimedDate));
        return {
            matches: hasMatch,
...
exports.verifyInstallationDate = verifyInstallationDate;
/**
...
exports.default = {
    ...
    extractTextWithFallback: exports.extractTextWithFallback,
    extractDatesFromText: exports.extractDatesFromText,
    validateInvoiceDateRange: exports.validateInvoiceDateRange,
    validateWarrantyByOCR: exports.validateWarrantyByOCR,
    verifyInstallationDate: exports.verifyInstallationDate,
